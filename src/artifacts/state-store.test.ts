...
describe('the state store contract', () => {
  ...
  it('moveReader: anonymous answers move to the signed-in reader; an existing signed-in one wins', async () => {
    ...
    expect(await s.moveReader('p', anon.key, sam)).toBe(2)
    const e = await s.entries('p')
    expect(e.every((x) => x.readerKey === sam.key)).toBe(true)
    expect(e.find((x) => x.slot === 'vote')?.value).toBe('sam')
    expect(e.find((x) => x.slot === 'form')?.value).toEqual({ a: 1 })
    expect(e.find((x) => x.slot === 'notes')?.writer.email).toBe('sam@example.com')
  })
